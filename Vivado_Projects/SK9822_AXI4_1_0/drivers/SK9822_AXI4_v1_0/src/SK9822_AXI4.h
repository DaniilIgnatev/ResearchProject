...
typedef struct { 
  union {
    ...
    struct {
      __IM  uint8_t TI         : 1;            // Transmission status
      __IOM uint8_t INSEL      : 1;            // Color  source selection
      __IOM uint8_t LOOP       : 1;            // Continuous transmission option
            uint8_t            : 5;
    } bit;
  } CSR;
  ...
  union {
    ...
    struct {
      __OM uint8_t ST          : 1;            // Start transmission command
           uint8_t             : 7;
    } bit;
  } TSR;
  ...
  union {
    ...
    struct {
      __IOM uint8_t INSEL       : 1;            // Global brightness input selection in full colored mode
            uint8_t             : 2;
      __IOM uint8_t GB          : 5;            // Global brightness value
    } bit;
  } GBCR;
  ...
  union {
    ...
    struct {
      __IOM uint8_t TIEN        : 1;            // Transmission interrupt enable
      __IM  uint8_t TI          : 1;            // Transmission interrupt status
      __OM  uint8_t CTI         : 1;            // Clear transmission interrupt
      __OM  uint8_t STI         : 1;            // Set transmission interrupt
            uint8_t             : 4;
    } bit;
  } ICSR;
} CSR_Type;
...
union {
  ...
  struct {
    __IOM  uint8_t BS           : 5;            // Individual brightness
           uint8_t              : 3;
    __IOM  uint8_t R            : 8;            // Red   
    __IOM  uint8_t G            : 8;            // Green   
    __IOM  uint8_t B            : 8;            // Blue  
  } bit;
} LED_Type;

void SK9822_reset(uint32_t BaseAddress);

bool SK9822_get_transmission_indication(uint32_t BaseAddress);

bool SK9822_get_color_source_selection(uint32_t BaseAddress);

void SK9822_set_color_source_selection(uint32_t BaseAddress, bool option);

bool SK9822_get_continuous_transmission(uint32_t BaseAddress);

void SK9822_set_continuous_transmission(uint32_t BaseAddress, bool on_off);

void SK9822_start_transmission(uint32_t BaseAddress);

bool SK9822_get_brightness_source_selection(uint32_t BaseAddress);

void SK9822_set_brightness_source_selection(uint32_t BaseAddress, bool option);

uint8_t SK9822_get_global_brightness(uint32_t BaseAddress);

void SK9822_set_global_brightness(uint32_t BaseAddress, uint8_t value);

bool SK9822_get_transmission_interrupt_enable(uint32_t BaseAddress);

bool SK9822_get_transmission_interrupt_status(uint32_t BaseAddress);

void SK9822_clear_transmission_interrupt(uint32_t BaseAddress);

void SK9822_set_transmission_interrupt(uint32_t BaseAddress);

LED_Type SK9822_get_LED(uint32_t BaseAddress, uint8_t id);

void SK9822_set_LED(uint32_t BaseAddress, uint8_t id, LED_Type data);

bool SK9822_get_R(uint32_t BaseAddress, uint8_t id);

void SK9822_set_R(uint32_t BaseAddress, uint8_t id, bool on_off);

bool SK9822_get_G(uint32_t BaseAddress, uint8_t id);

void SK9822_set_G(uint32_t BaseAddress, uint8_t id, bool on_off);

bool SK9822_get_B(uint32_t BaseAddress, uint8_t id);

void SK9822_set_B(uint32_t BaseAddress, uint8_t id, bool on_off);
 ...
 * @note
 * C-style signature:
 * 	void SK9822_AXI4_mWriteReg(u32 BaseAddress, unsigned RegOffset, u32 Data)
 *
 */
 ...
 *
 * @param   baseaddr_p is the base address of the SK9822_AXI4 instance to be worked on.
 *
 * @return
 *
 ...
 *
 */
XStatus SK9822_AXI4_Reg_SelfTest(void * baseaddr_p);
