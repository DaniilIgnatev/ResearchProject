...
void setup(){
    ...
    SK9822_TransmissionOptions options = init_SK9822_TransmissionOptionsType(full, global, brightness, false, false);
...
void loop()
{
  ...
  for (uint16_t id = 0; id < LED_number; id++)
  {
    ...
    SK9822_set_LED(SK9822_AXI4_LED_BASEADDR, id, led);

    SK9822_set_R(SK9822_AXI4_R_BASEADDR, id, led.bit.R > (255 / 2));
    SK9822_set_G(SK9822_AXI4_G_BASEADDR, id, led.bit.G > (255 / 2));
    SK9822_set_B(SK9822_AXI4_B_BASEADDR, id, led.bit.B > (255 / 2));
  }

  SK9822_start_transmission(SK9822_AXI4_SETTINGS_BASEADDR);
}
